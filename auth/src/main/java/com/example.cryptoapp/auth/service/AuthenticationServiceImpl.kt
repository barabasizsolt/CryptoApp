...
class AuthenticationServiceImpl : AuthenticationService {
    ...
    override fun loginWithEmailAndPassword(email: String, password: String): Flow<AuthResult> = callbackFlow {
        consumeTask(task = firebaseAuth.signInWithEmailAndPassword(email, password))
    }

    @OptIn(ExperimentalCoroutinesApi::class)
    override fun registerWithEmailAndPassWord(email: String, password: String): Flow<AuthResult> = callbackFlow {
        consumeTask(task = firebaseAuth.createUserWithEmailAndPassword(email, password))
    }
    ...
    override fun resetPassword(email: String): Flow<AuthResult> = callbackFlow {
        consumeTask(task = firebaseAuth.sendPasswordResetEmail(email))
    }
